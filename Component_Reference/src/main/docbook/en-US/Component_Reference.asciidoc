...
Use [sgmltag]+<a4j:repeat>+ and a collection of objects inside the [sgmltag]+<r:series>+ tag and specify what you want to be plotted on the x and y axis using [sgmltag]+<r:point>+ tag. The [sgmltag]+<a4j:repeat>+ approach can also be used at the [sgmltag]+<r:series>+ level.
