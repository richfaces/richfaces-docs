...
This book is a guide to the various components available in the RichFaces 4.5.0-SNAPSHOT framework. It includes descriptions of the role of the components, details on how best to use them, coded examples of their use, and basic references of their properties and attributes.
...
====
Refer to the *_JBoss EAP Administration and Configuration Guide_* for details on configuring JMS inJBoss EAP.
====
...
All the switchable components ( [sgmltag]+<rich:togglePanel>+, [sgmltag]+<rich:accordion>+ component and the [sgmltag]+<rich:tabPanel>+) can leverage the [sgmltag]+<rich:repeat>+ tag to dynamically create child components. This can be useful when the definition of the panel items is determined at run-time from a backing bean list.
...
The non-visual [sgmltag]+<rich:repeat>+ component is used to iterate through a data model. The component renders child content for every iteration according to the current object data.

The [sgmltag]+<rich:repeat>+ component extends the standard [classname]+UIRepeat+ component to allow partial updates within iterations while sending Ajax requests. The component acts as a base for all the data iteration components detailed in this chapter.

[[sect-Component_Reference-repeat-Basic_usage]]

==== Basic usage

The contents of the collection are determined using Expression Language ( [acronym]#EL#). The data model for the contents is specified with the [varname]+value+ attribute. The [varname]+var+ attribute names the object to use when iterating through the collection. This object is then referenced in the relevant child components. <<exam-Component_Reference-repeat-repeat_example>> shows how to use [sgmltag]+<rich:repeat>+ to maintain a simple table.

[[exam-Component_Reference-repeat-repeat_example]]

.[sgmltag]+<rich:repeat>+ example
====
...
<table>
    <tbody>
        <rich:repeat value="#{repeatBean.items}" var="item">
            <tr>
...
The [sgmltag]+<rich:repeat>+ component uses other attributes common to iteration components, such as the [varname]+first+ attribute for specifying the first item for iteration, and the [varname]+rows+ attribute for specifying the number of rows of items to display.
...
As [sgmltag]+<rich:dataTable>+ the component is based on the [sgmltag]+<rich:repeat>+ component, it can be partially updated with Ajax. Refer to <<sect-Component_Reference-repeat-Limited_views_and_partial_updates>> for details on partially updating the [sgmltag]+<rich:dataTable>+ component.
...
As [sgmltag]+<rich:dataGrid>+ the component is based on the [sgmltag]+<rich:repeat>+ component, it can be partially updated with Ajax. Refer to <<sect-Component_Reference-repeat-Limited_views_and_partial_updates>> for details on partially updating the [sgmltag]+<rich:dataGrid>+ component.
...
The [sgmltag]+<r:chart>+ tag encapsulate children tags which customizes the chart. The chart type is selected by [sgmltag]+<r:series>+ attribute. The chart has many attributes which can customize its look all of them are optional. Only requirements to use the [sgmltag]+<r:chart>+ are selection of chart type and pass at least one series of data - explained below.


[[exam-Component_Reference-chart_example]]
...
The [sgmltag]+<r:chart>+ component offers two ways how to pass a data into it - facelet iteration and creating data model object.

===== Facelet iteration

Use [sgmltag]+<r:repeat>+ and a collection of objects inside the [sgmltag]+<r:series>+ tag and specify what you want to be plotted on x and y axis using [sgmltag]+<r:point>+ tag. This approach is shown in basic usage example. The [sgmltag]+<r:repeat>+ approach can be used also at [sgmltag]+<r:series>+ level.

===== Create DataModel object
When facelet iteration is used the ChartDataModel object is created inside by ChartRenderer. The alternative way is to create ChratDataModel and pass it using [sgmltag]+<r:series>+ data attribute. Create instance one of the child classes of ChartDataModel - NumberChartDataModel, StringChartDataModel or DateChartDataModel (not fully supported). Select class according to the data type used on x-axis. Add valuest to the model using put method and pass it to the [sgmltag]+<r:series>+ using data attribute


If there is model passed using <r:series> data it is used, nested <r:point> tags are ignored. If data attribute is not used nested <r:point> tags are expected. 

[[sect-Component_Reference-chart-Look_customization]]

==== Chart look customization

The chart configuration is split into tags to make facelet code clearer. To configure axes, their min, max values and label use [sgmltag]+<r:xaxis>+ or [sgmltag]+<r:yaxis>+ tag. The [sgmltag]+<r:legend>+ allows to set up the position of legend and labels order in it.

To adjust chart component size you can use CSS class .richfaces-chart-container, to customize title use .richfaces-chart-title the placeholder. The chart itself is placed in div with class .richfaces-chart. 

[[sect-Component_Reference-chart-Interactivity_options]]

==== Interactivity options

The <r:chart> component does not create static chart only. 

* It allows to zoom line charts when [sgmltag]+<r:chart>+ attribute zoom is set true. To reset zoom you can use JavaScript API.
* You can also add functions to handle events fired by component. Event handlers are set up using proper <r:chart> attributes. They handle events fired by all series. If you want to handle event only fired by particular series, set up handlers using <r:series> attributes.

[[sect-Component_Reference-chart-chart_server-side_events]]

==== [sgmltag]+<r:chart>+ server-side events

* The [varname]+PlotClickEvent+ is fired when the user click a point in a chart. To set a listner use [varname]+ClickListener+ attribute.

[[sect-Component_Reference-chart-chart_client-side_events]]

==== [sgmltag]+<r:chart>+ client-side events

* The [varname]+plothover+ event points to the function to perform when mouse cursor is above the point in a chart.
* The [varname]+plotclick+ event points to the function to perform when the user click point in a chart.
* The [varname]+mouseout+ event points to the function to perform when the mouse cursor leaves the chart grid.

The plothover and plotclick handlers are given an event object containing data of point which fired an event.
...
To access the component using jQuery widget use componentID + chart
