...
This book is a guide to the various components available in the RichFaces 4.5.17-SNAPSHOT framework. It includes descriptions of the role of the components, details on how best to use them, coded examples of their use, and basic references of their properties and attributes.
...
[function]+selectRows(index|range)+:: Select one or multiple rows, accepts a number (rowIndex) or an array ([firstRowIndex, lastRowIndex]).
[function]+deselectRow(index)+:: Deselect the row that is currently selected.
