...
The auto-complete box is a standard JSF [classname]+UIInput+ control.
...
.+client+ mode
====
The prefix is normally ignored in +client+ mode. In this mode, the component requests the suggestion list only once, and performs filtering on the client.
====
...
====
A mode should be always selected for particular use case: if it is assumed that Autocomplete will handle tons of data, use cachedAjax or ajax mode. If it will allow to select just between few options, e.g. tens, you can use client mode. Be aware that ajax mode can overload your server with number of requests.
====
...
Users can type into the text field to enter a value, which also searches through the suggestion items in the suggestion box. By default, the first suggestion item is selected as the user types. This behavior can be deactivated by setting [code]+selectFirst="false"+.

Setting [code]+autoFill="true"+ causes the suggestion box to fill the text field with a matching suggestion as the user types.
...
[function]+search()+:: Show the pop-up list of completion values.
[function]+close()+:: Hide the pop-up list.
[function]+enable()+:: Enables an input field and autocompletion functionality.
[function]+disable()+:: Disables an input field and autocompletion functionality.

[[sect-Component_Reference-autocomplete-Reference_data]]

==== Reference data

* [parameter]+component-type+: [classname]+org.richfaces.ui.Autocomplete+
* [parameter]+component-class+: [classname]+org.richfaces.ui.input.autocomplete.UIAutocomplete+
* [parameter]+component-family+: [classname]+javax.faces.Input+
* [parameter]+renderer-type+: [classname]+org.richfaces.ui.AutocompleteRenderer+
* [parameter]+handler-class+: [classname]+org.richfaces.ui.input.autocomplete.AutocompleteHandler+
