...
The RichFaces 4 framework is made up of a single tag library: the [classname]+r+ library.
...
[function]+moveUp()+:: Move the currently selected item(s) up one step.
[function]+moveDown()+:: Move the currently selected item(s) down one step.
[function]+moveFirst()+:: Move the currently selected item(s) to the top of the list.
[function]+moveLast()+:: Move the currently selected item(s) to the bottom of the list.
[function]+getSelected()+:: Find and return the list of orderingList elements that are selected.
[function]+selectItem(item)+:: Select the given +item+.
[function]+selectAll()+:: Select all items in the orderingList
[function]+unselectAll()+:: Un-select all items in the orderingList
[function]+unSelectItem(item)+:: Un-select the given +item+.
[[sect-Component_Reference-orderingList-Reference_data]]

==== Reference data

* [parameter]+component-type+: [classname]+org.richfaces.select.OrderingList+
* [parameter]+component-class+: [classname]+org.richfaces.ui.select.UIOrderingList+
* [parameter]+component-family+: [classname]+org.richfaces.SelectMany+
* [parameter]+renderer-type+: [classname]+org.richfaces.ui.select.OrderingListRenderer+
...
[function]+getSourceWidget()+:: Returns the javascript list object backing the [sgmltag]+<r:pickList>+ source list. This list can be used to select/unselect item(s).
[function]+getTargetWidget()+:: Returns the javascript list object backing the [sgmltag]+<r:pickList>+ target list. This list can be used to select/unselect item(s).
[function]+addItems()+:: Add the currently selected items to the target list, removing them from the source list.
[function]+addAllItems()+:: Add all the source items to the target list, removing them from the source list.
[function]+removeItems()+:: Remove the currently selected items from the target list, adding them to the source list.
[function]+removeAllItems()+:: Remove all the source items from the target list, adding them to the source list.
[[sect-Component_Reference-pickList-Reference_data]]

==== Reference data

* [parameter]+component-type+: [classname]+org.richfaces.select.PickList+
* [parameter]+component-class+: [classname]+org.richfaces.ui.select.UIPickList+
* [parameter]+component-family+: [classname]+org.richfaces.SelectMany+
* [parameter]+renderer-type+: [classname]+org.richfaces.ui.select.PickListRenderer+
