...
+
----
mvn archetype:generate -DarchetypeGroupId=org.richfaces.archetypes -DarchetypeArtifactId=richfaces-archetype-simpleapp -DarchetypeVersion=4.5.0-SNAPSHOT -DgroupId=org.docs.richfaces -DartifactId=new_project
----
+
...
====
The ability to prepare the project for Eclipse and export it using Maven is deprecated in RichFaces 4.5.0-SNAPSHOT. The process does not support JBoss integration-specific features, such as JSF Facets.
====
...
Both [code]+some:jquery.js+ and [code]+anotherich:jquery.js+ are mapped to [code]+final:jquery.js+:
...
----
.rf-p{
   background-colorich:'#{richSkin.generalBackgroundColor}';
   colorich:'#{richSkin.panelBorderColor}';
   border-width:1px;
   border-style:solid;
   padding:1px;
}
...
.rf-p-b{
   ...
   colorich:'#{richSkin.generalTextColor}';
