...
The RichFaces framework is a rich component library for JavaServer Faces ( [acronym]#JSF#). The framework extends the Ajax capabilities of JSF with advanced features for enterprise web application development.
...
====
Some of the dependencies are part of the Java EE 6 specification and thus it is not necessary to include them in projects running on Java EE applications servers.  It is still necessary to include them on servlet containers.

This does not apply to dependencies on the Servlet API, JSP API and EL API, as these are APIs are integral parts of both application servers and servlet containers.
====
...
Other development environments, such as [productname]#Idea# or [productname]#NetBeans#, could also be used for RichFaces development, but are not detailed in this book.
